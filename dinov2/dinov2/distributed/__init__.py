...
def enable(*, set_cuda_current_device: bool = True, overwrite: bool = False, allow_nccl_timeout: bool = False):
    ...
    # In dinov2/distributed/__init__.py, inside the enable function, before dist.init_process_group
    # print(f"DEBUG {torch_env.local_rank}: MASTER_ADDR={os.environ.get('MASTER_ADDR')}, MASTER_PORT={os.environ.get('MASTER_PORT')}, RANK={os.environ.get('RANK')}, WORLD_SIZE={os.environ.get('WORLD_SIZE')}")
    dist.init_process_group(backend="nccl", device_id=torch.device(f"cuda:{torch_env.local_rank}"))
    dist.barrier()
